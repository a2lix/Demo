...
a2lix_lib.formCollection = (() => {
    const init = (collectionsSelector = 'form div[data-prototype]', manageRemoveEntry = true) => {
    ...
    const processCollectionElt = (collectionElt, manageRemoveEntry) => {
        ...
        collectionElt.addEventListener('click', (evt) => configureCollectionElt(evt, manageRemoveEntry))
    }
    ...
    const appendEntryAddLink = (collectionElt) => {
        ...
        const entryAddLink = getButtonElt(`Add ${entryLabel}`, 'add', 'btn btn-primary btn-sm mt-2')
        collectionElt.appendChild(entryAddLink)
    }

    const appendEntryRemoveLink = (collectionElt) => {
        const entryRemoveLink = getButtonElt('Remove', 'remove', 'btn btn-danger btn-sm');

        [...collectionElt.children].filter(entryElt => !entryElt.hasAttribute('data-entry-action')).forEach(entryElt => {
    ...
    const getTemplateContent = (entryPrototype, entryIndex) => {
        ...
        const entryHtml = entryPrototype.replace(/__name__label__/g, `!New! ${entryIndex}`)
